#!/usr/bin/env python3
"""ADKDD 2023 ページから論文リスト（title / section / doi）を抽出して JSON に保存する。"""
...
DEFAULT_URL = "https://www.adkdd.org/papers/2023"
...
def main() -> None:
    """ADKDD 2023 論文リストを JSON として保存する。"""
    parser = argparse.ArgumentParser(description="Build ADKDD 2023 paper list JSON from web page")
    parser.add_argument("--url", default=DEFAULT_URL, help="ADKDD 2023 papers page URL")
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("results/ADKDD-23/accepted_papers.json"),
    ...
    payload = {
        ...
        "year": 2023,
        "source_url": args.url,
        "dblp_query": "toc:db/conf/kdd/adkdd2023.bht:",
        "papers": records,
    }
