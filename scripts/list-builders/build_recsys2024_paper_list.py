#!/usr/bin/env python3
"""RecSys 2024 Accepted Contributions ページから論文リストを抽出して JSON に保存する。"""
...
DEFAULT_URL = "https://recsys.acm.org/recsys24/accepted-contributions/"
...
class RecSys2024Parser(HTMLParser):
    """RecSys 2024 Accepted Contributions ページ向けパーサー。"""
...
def parse_records(html: str) -> list[dict]:
    ...
    parser = RecSys2024Parser()
    parser.feed(html)
    return parser.records


def main() -> None:
    """RecSys 2024 の accepted contributions を JSON へ保存する。"""
    parser = argparse.ArgumentParser(description="Build RecSys 2024 accepted papers JSON")
    parser.add_argument("--url", default=DEFAULT_URL, help="Accepted contributions page URL")
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("results/RecSys-24/accepted_papers.json"),
    ...
    payload = {
        ...
        "year": 2024,
        "source_url": args.url,
        "dblp_query": "toc:db/conf/recsys/recsys2024.bht:",
        "papers": records,
    }
